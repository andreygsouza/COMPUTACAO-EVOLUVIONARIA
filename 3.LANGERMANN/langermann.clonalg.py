...
def main():
    ...
    for rep in range(Nrep):
        ...
        nAleatorio = 5      #numero de anticorpos de menor afinidade que devem ser substuidos
        ...
        VarMin=-513      #Limite inferior da função
        VarMax= 513    #Limite superio da função------------------------------------------------------------
        #Função custo e
        ...
        for ite in range(MaxIt):
            ...
            MinSol_Cost = min(selected_Cost) 
        
            if MaxSol_Cost == 0:  
                MaxSol_Cost =1
                #Seleciona o valor mais baixo do vetor para normalizar os demais
            ...
            for it in range(nSelecao):
                #Maturação
                Maturate_Rate[it,0] = abs((BestSol_Cost[it,:]/50)*np.exp(-Rho*BestSol_Cost[it,:]))
    ...
    np.savetxt("clonalg_lagermann.csv", resultados, delimiter=",")
    ...
    plt.xlabel("Iteração")
    plt.ylabel("Custo")
    plt.title("Custo por Iteração - Função Lagermann: Clonalg")
